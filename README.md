...
Being built. The contracts, the note series, the coupon and maturity runs, the index model, the API, the investor screens and the Steward agent are live on Hedera testnet; the API's three metered endpoints are gated with x402 and settle through Blocky402. The oracle and the Adjuster commands still print what they will do instead of doing it, and later tickets fill them in one at a time.
