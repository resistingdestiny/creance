...
/// @notice Test-only token that calls back into a target contract during a
/// transfer, so
...
contract ReentrantSettlementToken is ERC20 {
    ...
    function _update(address from, address to, uint256 value) internal override {
        ...
        if (callbackTarget != address(0) && !_entered) {
            _entered = true;
            // Bubble the revert up so the test sees the guard, not a swallowed
            // failure.
            (bool ok, bytes memory reason) = callbackTarget.call(payload);
            _entered = false;
            if (!ok) {
                assembly {
