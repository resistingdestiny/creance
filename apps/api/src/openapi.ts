  ...
  return {
    ...
    paths: {
      ...
      '/v1/subscribe': {
        post: {
          ...
          summary: 'Put principal into a series',
          description: [
            'Free. Approves the vault from the paying account where the standing',
            'allowance does not already cover the amount, then writes the subscription,',
            'and answers with both transactions and what the vault says the holder was',
            'carrying before and after.',
            '',
            'The amount is added to whatever the holder already has. It is not a top up',
            'to a total: a holder carrying 50000000000 who subscribes 25000000000 ends',
            'up with 75000000000. So the call is not idempotent, and a retry after a',
            'timeout subscribes again. `subscription_before` and `subscription_after`',
            'are how a caller tells what its call did.',
            '',
            'The money comes out of the API account, not the holder. The vault pulls',
            'the settlement token from whoever calls it, and the account holding the',
            'subscription role on this deployment is the API account, so a subscription',
            'is the API paying for the holder and recording the holder as the',
            'subscriber. That is why `holder` is held to an account this deployment',
            'already speaks for: it names who is credited, not who pays. A deployment',
            'with no key for the paying account answers 503 and serves every read as',
            'before.',
            '',
            'Three reads happen before anything is signed, because an approve through',
            'the HTS ERC-20 facade costs about 1.7 HBAR and it is the first of the two',
            'calls: the series in the vault, the standing subscription, and the paying',
            'balance. A subscription that was always going to be refused is refused',
          ...
          responses: {
            '201': {
              description: 'The subscription, and the transactions that made it.',
              content: {
                'application/json': { schema: { $ref: '#/components/schemas/Subscription' } },
              },
            },
            '400': problemResponse(
              '`validation_failed` for a missing field, `amount_invalid` for anything that is not a whole number of minor units above nought, `holder_unknown` for an account this deployment does not pay for, `holder_invalid` for an empty address.',
            ),
            '404': problemResponse(
              '`series_not_found`: no series with that id on this network. `series_not_open`: the vault has never opened that series.',
            ),
            '409': problemResponse(
              '`series_matured`: the series has reached maturity and takes no more principal. `settlement_balance_short`: the paying account holds less than the amount, and nothing was sent. `vault_paused`.',
            ),
            '502': problemResponse(
              '`subscription_role_missing`, `settlement_transfer_failed` or `chain_write_failed`: the chain refused the write and nothing was subscribed.',
            ),
            '503': problemResponse(
              '`subscription_writes_unavailable`: this deployment reads the series but holds no key for the account that pays.',
            ),
          },
        },
      },
    ...
    components: {
      ...
      schemas: {
        ...
        SubscribeRequest: {
          type: 'object',
          required: ['series', 'holder', 'amount'],
          properties: {
            series: {
              type: 'string',
              description: 'The series to subscribe to, by label, bytes32 id or occupation group.',
              example: 'ODI-COMP-2026-01',
            },
            holder: {
              type: 'string',
              description:
                'The account to credit, by its demo role, its Hedera account id or its EVM address. It is the account the vault records as the subscriber and the account a redemption at maturity pays; it is not the account that pays now.',
              example: 'investor-1',
            },
            amount: {
              type: 'string',
              description:
                'How much to add, an integer in the settlement asset smallest unit. 25,000 TUSD is 25000000000. It is added to what the holder already has rather than being a total.',
              example: '25000000000',
            },
          },
        },
        Subscription: {
          type: 'object',
          description:
            'A subscription as the vault now holds it. Every amount is Money, and the two subscription figures are read from the chain before and after the write rather than computed here.',
          required: [
            'network',
            'series_id',
            'holder',
          ...
          properties: {
            ...
            series_key: { type: 'string' },
            holder: { $ref: '#/components/schemas/MarketParty' },
            // The account that paid and signed. The money left this account and
            // not the holder, which is the one thing about a subscription here
            // that a reader should not have to guess. It carries no description
            // of its own because a sibling of `$ref` is ignored in 3.0, and the
            // operation says it in full.
            paid_by: { $ref: '#/components/schemas/MarketParty' },
            amount: { $ref: '#/components/schemas/Money' },
            subscription_before: { $ref: '#/components/schemas/Money' },
            subscription_after: { $ref: '#/components/schemas/Money' },
            transactions: {
              type: 'object',
              description:
                'The two calls, in the order they were made. `approve` is null where the standing allowance already covered the amount and none was sent.',
              properties: {
                approve: { type: 'string', nullable: true },
                subscribe: { type: 'string' },
              },
            },
            gas_used: { type: 'string', example: '141378' },
            hashscan: { type: 'string' },
          },
        },
        OfferRequest: {
